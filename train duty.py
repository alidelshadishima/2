'''
This code is used to train the default network for the baseline/incoming duties.
The baseline duties:
...
def evaluate(model, test_loader, label, save_flag, index):
    ...
    if (save_flag == True):
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/duty'+str(index)+'.t1')
    best_acc = correct
...
# load the train and test data for indicator duty
...
fit(cnn, trainloader)
evaluate(cnn, testloader, test_label, True, dutyID)
...
# np.save('feature_data/duty'+str(dutyID)+'_train_feature_ind.npy', train_feature)
# np.save('feature_data/duty'+str(dutyID)+'_test_feature_ind.npy', test_feature)
# # save labels
# print("Saving train and test labels")
# np.save('feature_data/duty'+str(dutyID)+'_train_label_ind.npy', train_label)
# np.save('feature_data/duty'+str(dutyID)+'_test_label_ind.npy', test_label)

#load features
#feature = np.load('feature_data/feature_ind'+str(dutyID)+'.npy')
