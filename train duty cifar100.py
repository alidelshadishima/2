...
def evaluate(model, test_loader, label, save_flag, index, i):
    ...
    if (save_flag == True):
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/cifar100_duty'+str(index)+'.t'+str(i))
    best_acc = correct
...
if __name__ == "__main__": 
    for i in range(10):
        ...
        # duty 1: (binary) 18: [8, 58, 90, 13, 48] 19: [81, 69, 41, 89, 85]
        # duty 2: (binary) 5: [40, 39, 22, 87, 86] 6: [20, 25, 94, 84, 5]
        # duty 3: (multi)
        # duty 4: (all)
        base_duty_list = np.array([ [8, 58, 90, 13, 48, 81, 69, 41, 89, 85], 
        ...
        for idx in range(len(base_duty_list)):
            ...
            if idx == 2:
                ...
                trainloader, testloader, train_label, test_label = CIFAR_multi_indicator_dataset('CIFAR100', base_duty_list[idx], args)
                c = 11
            elif idx == 3:
                ...
                trainloader, testloader, train_label, test_label = CIFAR_indicator_dataset('CIFAR100', base_duty_list[idx], args)
                c = 2
