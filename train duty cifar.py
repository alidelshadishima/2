...
def evaluate(model, test_loader, label, save_flag, index):
    ...
    if (save_flag == True):
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/cifar10_duty'+str(index)+'.t10')
    best_acc = correct
...
if __name__ == "__main__": 
    ...
    # print(subset)
    # # choose the first 15 combinations as base duties indicators
    # base_duty_list = subset[0:15]
    # # save the list
    # np.save('list/base_duty_3.npy', base_duty_list)
    # # save the entire shuffle list
    # np.save('list/base_duty_3_full.npy', subset)
    
    # load the list
    # subset = np.load('list/base_duty_3_full.npy')
    # base_duty_list = np.load('list/base_duty_3.npy')
    
    # duty 0-7
    # base_duty_list = np.array([ [1,3,8], [3,8,9], [3,4,8], [0,3,9], [0,5,9], [0,2,9], [5,6,7], [2,6,7] ])
    # duty 3b and 5b
    # base_duty_list = np.array([ [0,3,8], [0,7,9] ])
    
    # 1 object identification (duty 10-12)
    # base_duty_list = np.array([ [1], [9], [7] ])
    
    # 3 object identification (duty 20-22)
    # base_duty_list = np.array([ [1,3,8], [0,3,9], [2,6,7] ])
    
    # 10-class classification
    base_duty_list = np.array([ [10] ])
        
    # load CIFAR10 indicator dataset
    for idx in range(len(base_duty_list)):
        print(base_duty_list[idx])
        trainloader, testloader, train_label, test_label = CIFAR10_indicator_dataset('CIFAR10', base_duty_list[idx], args)
        # trainloader, testloader, train_label, test_label = CIFAR10_multi_indicator_dataset('CIFAR10', base_duty_list[idx], args)
        ...
        # train and evaluate on the indicator duty
